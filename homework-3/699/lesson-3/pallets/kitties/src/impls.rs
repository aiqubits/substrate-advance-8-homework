...
mod impls {
    impl<T: Config> Pallet<T> {
        ...
        // mint kitty
        fn mint_kitty(owner: &T::AccountId, data: [u8; 16]) -> DispatchResult {
        ...
        // bid for kitty
        pub fn bid_kitty(bidder: T::AccountId, kitty_id: u32, price: BalanceOf<T>) -> DispatchResult {
            ensure!(<Kitties<T>>::contains_key(&kitty_id), Error::<T>::InvalidKittyId);
            ...
            Self::deposit_event(Event::<T>::KittyBid {
                bidder: bidder.clone(),
                kitty_id,
                price,
            });
